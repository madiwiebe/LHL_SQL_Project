...
Step 1: Define "revenue impact" as "the net increase or decrease in revenue generated on an annual basis".

Step 2: Determine the yearly revenue generated by each city/country.

Step 3: Determine whether there is enough data to analyze revenue increase or decrease.
...
Country:
	...
	SELECT	DISTINCT(cpr.country),
			EXTRACT(YEAR FROM cpr.date) AS "year",
			SUM(cpr.revenue) OVER(PARTITION BY cpr.country, EXTRACT(YEAR FROM cpr.date)) AS yearly_revenue
	FROM country_product_revenue cpr
	GROUP BY cpr.country, "year", cpr.revenue
	ORDER BY "year" ASC, yearly_revenue DESC
	;
...
City:
	...
	SELECT	DISTINCT(cpr.city),
			EXTRACT(YEAR FROM cpr.date) AS "year",
			SUM(cpr.revenue) OVER(PARTITION BY cpr.city, EXTRACT(YEAR FROM cpr.date)) AS yearly_revenue
	FROM city_product_revenue cpr
	GROUP BY cpr.city, "year", cpr.revenue
	ORDER BY "year" ASC, yearly_revenue DESC
 	;

Answer:

Within the database, there are records only for the years 2016 and 2017. Of those records, there are only two countries that appear in both years (United States and Canada) and there are only two cities that appear in both years (Mountain View and New York). The revenue impact could be calculated as a percent increase for these four records, but for all other records we can only collect the total revenue generated by each country and city.
